...
import { BuyCreditsRequest$outboundSchema, BuyCreditsResponseBody$inboundSchema, } from "../models/buycreditsop.js";
 ...
 */
export function billingBuyCredits(client, request, options) {
    return new APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = safeParse(request, (value) => BuyCreditsRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result] = await M.match(M.json(200, BuyCreditsResponseBody$inboundSchema), M.fail([400, 401, 402, 403, 404, "4XX"]), M.fail([500, "5XX"]))(response, req);
    if (!result.ok) {
        return [result, { status: "complete", request: req, response }];
    }
    return [result, { status: "complete", request: req, response }];
}
//# sourceMappingURL=billingBuyCredits.js.map