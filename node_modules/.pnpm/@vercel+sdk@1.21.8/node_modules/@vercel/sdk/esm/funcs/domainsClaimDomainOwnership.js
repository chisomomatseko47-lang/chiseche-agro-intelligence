...
import { ClaimDomainOwnershipRequest$outboundSchema, ClaimDomainOwnershipResponseBody$inboundSchema, } from "../models/claimdomainownershipop.js";
 ...
 */
export function domainsClaimDomainOwnership(client, request, options) {
    return new APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = safeParse(request, (value) => ClaimDomainOwnershipRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result] = await M.match(M.json(200, ClaimDomainOwnershipResponseBody$inboundSchema), M.fail([400, 401, 403, 404, "4XX"]), M.fail("5XX"))(response, req);
    if (!result.ok) {
        return [result, { status: "complete", request: req, response }];
    }
    return [result, { status: "complete", request: req, response }];
}
//# sourceMappingURL=domainsClaimDomainOwnership.js.map